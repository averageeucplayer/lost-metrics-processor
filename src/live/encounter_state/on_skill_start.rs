...
impl EncounterState {
    ...
    pub fn on_skill_start(
        ...
        let mut source_entity = self.get_source_entity(source_id).clone();
        source_entity.guess_is_player(skill_id);
        ...
        self.log_skill_cast(entity.name.clone(), skill_id, relative_timestamp);
        ...
        if entity.entity_type == EntityType::Player && skill_id > 0 {
            self.new_cast(entity.id, skill_id, skill_summon_sources, timestamp);
        }
    ...
    fn get_or_create_entity_with_skill_stats(
        ...
        self.encounter.entities.entry(source_entity.name.clone())
            .or_insert_with(|| {
