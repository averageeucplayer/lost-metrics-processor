...
where
    ...
    EE: EventEmitter,
    ES: EncounterService {
    pub fn on_new_npc_summon(&self, data: &[u8], state: &mut EncounterState) -> anyhow::Result<()> {

        let packet = parse_pkt1(&data, PKTNewNpcSummon::new)?;
            
        let (hp, max_hp) = get_current_and_max_hp(&packet.npc_struct.stat_pairs);
        let entity = self.trackers.borrow_mut().entity_tracker.new_npc_summon(packet, max_hp);
        info!(
            "new {}: {}, eid: {}, id: {}, hp: {}",
